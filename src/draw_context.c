...
{
	draw_context->image.img_ptr = mlx_new_image(draw_context->mlx_ptr,
			draw_context->window_x, draw_context->window_y);
	draw_context->image.image = mlx_get_data_addr(draw_context->image.img_ptr,
			&draw_context->image.bits_per_pixel, &draw_context->image.size_line,
			&draw_context->image.endian);
}
...
int		pixel_put_image(void *mlx_ptr, void *win_ptr, int x, int y, int color)
{
	...
	(void)mlx_ptr;
	(void)win_ptr;
	if (x >= 0 && x < draw_context->window_x && y >= 0 && y >= 0 && y <
	        draw_context->window_y)
	{
